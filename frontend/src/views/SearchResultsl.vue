<template>
    ...
    <div v-if="field==='section'" class="sections">
        ...
        <div v-for="section in results" v-bind:key="section.id" class="section-item">
      ...
      <p v-else>No results for search "{{ search }}"</p>
     </div>
     <div v-if="field === 'book' || field === 'author'">
      <p class="pwhite">Your search results for "{{ search }}" under "{{ field }}" are given below..</p>
      <div v-if="results.length > 0" class="book-grid">
        <div v-for="book in results" :key="book.id" class="book-item" @click="viewDetailsLib(book.id)">
          ...
          <button class="button1" @click="viewDetailsLib(book.id)">View Details</button>
        </div>
      </div>
      <p v-else>No results for search "{{ search }}"</p>
    </div>
  ...
  export default {
    ...
    getSearchResults(){
      ...
      if (searchResults) {
        ...
        axios.get('http://127.0.0.1:5000/searchl',
        {
        headers: {
